...
import com.cubeia.firebase.clients.java.connector.WebSocketConnector;
 ...
 *
 */
public class WebSocketConnectorClient implements IOClient {
	...
	private static Executor executor = Executors.newCachedThreadPool(new NamedThreadFactory("websocket-connector-client"));
	...
	public void connect(InetSocketAddress host, boolean processLobby) {
		...
		try {
			connector = new WebSocketConnector(host.getHostName(), host.getPort(), "/socket", false, -1);
