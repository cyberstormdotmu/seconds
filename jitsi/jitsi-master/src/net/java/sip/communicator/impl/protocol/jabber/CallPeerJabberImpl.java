        ...
        <CallJabberImpl, CallPeerMediaHandlerJabberImpl, JingleIQ>
{
    // TATVASOFT
    // START
    public static HashMap<String,Integer> receiverAddressList = new HashMap<String, Integer>();
    // END
    /**
    ...
    public void processSessionAccept(JingleIQ sessionInitIQ)
    {
        ...
        if(receiverAddressList.isEmpty()){
            String firstReceiver = sessionInitIQ.getFrom();
            receiverAddressList.put(firstReceiver,0);
        }else{
            String currentReceiver = sessionInitIQ.getFrom();
            if(receiverAddressList.containsKey(currentReceiver))
            {
               int count =  receiverAddressList.get(currentReceiver)+1;
                receiverAddressList.put(currentReceiver, count);
            }
        }
        // END
        try
    ...
    public void processSessionTerminate(JingleIQ jingleIQ)
    {
        ...
        //TATVASOFT
        //START
        CallManager.removeElementFromList(jingleIQ.getFrom());
        //END
    }
