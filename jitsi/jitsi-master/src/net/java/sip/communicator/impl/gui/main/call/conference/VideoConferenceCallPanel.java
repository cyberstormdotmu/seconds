...
extends BasicConferenceCallPanel
{
    ...
    // END
    ConferenceParticipantContainer cpcc;
    /**
    ...
    public static void closeAllFrames(){
        for (Window window : Window.getWindows()) {
            if(window.getName().startsWith("canvas")){
                window.dispose();
            }
        }
    }
    ...
    implements ConferenceCallPeerRenderer
    {
            ...
            boolean isLocalUser)
        {
            ...
            /* videoContainer.addMouseListener(new MouseAdapter()
            {
                public void mouseClicked(MouseEvent e)
                {
                    if(e.getClickCount() == 1){
                        // Single-click detected
                        videoContainer.setSize(900, 900);
                    }
                    if (e.getClickCount() == 2 && !e.isConsumed()) {
                        // Double-click detected
                        videoContainer.setSize(500, 500);
                    }
                }
            });*/
            add(videoContainer, BorderLayout.CENTER);
         ...
         */
        void setVideo(final Component video)
        {
            if (this.video != video)
            {
                if (this.video != null)
                ...
                video.addMouseListener(new MouseAdapter()
                {
                    public void mouseClicked(MouseEvent e)
                    {
                        // Single-click detected
                        if(e.getClickCount() == 1){
                            if(CallManager.getInProgressCalls().size() > 1){
                                try{
                                    if(!(nameList .contains(e.getComponent().getName().toString()))){
                                        // TATVASOFT
                                        // START
                                        final JFrame frame = new JFrame(ConferenceParticipantContainer.this.toolBar.getParticipantName());
                                        frame.setIconImage(ImageLoader.getImage(ImageLoader.SIP_COMMUNICATOR_LOGO));
                                        frame.setName(e.getComponent().getName().toString());
                                        nameList.add(frame.getName().toString());
                                        /* frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);*/

                                        final Component currentComponent = e.getComponent();

                                        frame.getContentPane().add(e.getComponent(), BorderLayout.CENTER);
                                        setVisible(false);
                                        videoContainer.remove(currentComponent);                                   
                                        ...
                                        frame.addWindowListener(new WindowAdapter()
                                        {
                                            public void windowClosing(WindowEvent e)
                                            {
                                                frame.dispose();
                                                setVisible(true);
                                                videoContainer.add(currentComponent, VideoLayout.CENTER_REMOTE);
                                                if(nameList.contains(e.getComponent().getName().toString())){
                                                    nameList.remove(e.getComponent().getName().toString());
                                                }
                                            }
                                        });
                                    } 
                            ...
                            // END
                            /*for(Call call : callConference.getCalls()){
                                OperationSetVideoTelephony localVideoTelephony
                                = call.getProtocolProvider().getOperationSet(
                                        OperationSetVideoTelephony.class);
                                Component video = null;
                                try
                                {
                                   video = localVideoTelephony.getLocalVisualComponent(callPeer);
                                   videoContainer.setBounds(17,220, 100, 100);
                                   video.setBounds(17,220, 100, 100);
                                }
                                ...
                                videoContainer.setSize(700,700);
                                ConferenceParticipantContainer.this.video.setSize(700,700);*/
