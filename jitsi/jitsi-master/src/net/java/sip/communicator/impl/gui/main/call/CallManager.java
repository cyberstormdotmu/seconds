...
public class CallManager
{
    ...
    //Call History
    // START
    public static HashMap<String,Integer> callerList = new HashMap<String,Integer>();
   ...
   /**
        ...
        extends CallChangeAdapter
    {
        ...
        public void incomingCallReceivedInEventDispatchThread(CallEvent ev)
        {
            ...
            // TATVASOFT
            // START
            Iterator<? extends CallPeer> callpeerItr = sourceCall.getCallPeers();
            CallPeer callpeer = null;
            while(callpeerItr.hasNext())
            {
                callpeer = callpeerItr.next();
            }
            
            String currentCaller = callpeer.getAddress();
            currentAddress = callpeer.getAddress();
            if(!callerList.isEmpty()){
                if(callerList.containsKey(currentCaller)){
                    int newCount = callerList.get(currentCaller)+1;
                    CallManager.callerList.put(currentCaller, newCount);
                    CallManager.answerCallInFirstExistingCall(sourceCall);
                    /*if(ev.isVideoCall()){
                        if(!ev.isDesktopStreaming()){
                        }else{
                            CallManager.answerCallInFirstExistingCall(sourceCall);
                        }
                    }else{
                    ...
                    /*callerList.put(currentCaller,count);*/
        ...
        public void run()
        {
            ...
            if (call != null)
            {
                ...
                while (peerIter.hasNext()){
                    callPeer = peerIter.next();// TATVASOFT
                    peers.add(callPeer);
                    // TATVASOFT
                    // START
                    removeAddress = callPeer.getAddress();
                    removeElementFromList(removeAddress);
                    // END
                }
            }
    ...
    /* TATVASOFT
	  *
	  * Created to call private createDesktopSharing method using this public method by class name.
	  * 
	  * Start
      */
    ...
    // TATVASOFT
    // START
    public static void removeElementFromList(String from){
        System.gc();
        try{
            VideoConferenceCallPanel.closeAllFrames();
            if(!VideoConferenceCallPanel.nameList.isEmpty()){
                VideoConferenceCallPanel.nameList.clear();
            }
        }catch(Exception e){
            e.printStackTrace();
        }
        try
        {
            for (String key:callerList.keySet()){
                if(key.equals(from)){
                    int remainingCall = callerList.get(key);
                    remainingCall = remainingCall - 1;
                    callerList.put(key, remainingCall);
                        if(remainingCall == 0){
                            callerList.remove(key);
                            /*isNewCall= true;*/
                  }
                }
            }
            
            for(String key: CallPeerJabberImpl.receiverAddressList.keySet()){
                if(key.equals(from)){
                    int remainingCall = CallPeerJabberImpl.receiverAddressList.get(key);
                    if(remainingCall == 0){
                            CallPeerJabberImpl.receiverAddressList.remove(key);
                        }
                    else{
                        if(remainingCall>1){
                            remainingCall = remainingCall - 2;
                        }else{
                            remainingCall = remainingCall - 1;
                        }
                        CallPeerJabberImpl.receiverAddressList.put(key, remainingCall);
                    }
                }
            }
            
            if((CallPeerJabberImpl.receiverAddressList.isEmpty() || getInProgressCalls().size()==0) && DefaultTreeContactList.renderReference!=null){
                DefaultTreeContactList.renderReference.enableAllButtons();
                //DefaultTreeContactList.renderReference = null;
            } 
        }
        catch (Exception e)
        {
