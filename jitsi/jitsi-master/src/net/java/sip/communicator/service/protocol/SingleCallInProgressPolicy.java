                    ...
                    peerIter.hasNext();)
            {
                        ...
                        && !CallPeerState.isOnHold(peerState))
                {
                    ...
                    {   
                        // TATVASOFT
                        // START
                        if(!CallPeerJabberImpl.receiverAddressList.isEmpty()){
                           if(CallPeerJabberImpl.receiverAddressList.containsKey(peer.getAddress()) && CallPeerJabberImpl.receiverAddressList.get(peer.getAddress())==1){
                               
                                telephony.putOffHold(peer);
                                int count = CallPeerJabberImpl.receiverAddressList.get(peer.getAddress())+1;
                                CallPeerJabberImpl.receiverAddressList.put(peer.getAddress(), count);
                                if(CallPeerJabberImpl.receiverAddressList.containsKey(CallManager.currentAddress)){
                                    telephony.putOffHold(peer);
                                }
                            ...
                            if(!CallManager.callerList.isEmpty()){
                                
                                if(CallManager.callerList.containsKey(CallManager.currentAddress) && CallManager.callerList.get(CallManager.currentAddress)==2){
                                    telephony.putOffHold(peer);
                                }else{
                                   telephony.putOnHold(peer);
                                   CallManager.callerList.put(CallManager.currentAddress,1);
                                }
