...
public class CommandNotifier {
	...
	private boolean handleLoggedInCommand(String command) {
        ...
        } else if (args[0].equalsIgnoreCase( Commands.SEND_TO_TABLE )) {
        	int tableId = Integer.parseInt(args[1]);
        	String msg = command.substring(args[0].length() + args[1].length()+2);
        	send(tableId, msg);
	...
	private void displayCommands() {
		String cmdList = 
			...
			"\t table <TID> <msg> -Send a string as a game transport packet to a table\n"+
	...
	private void send(int tableId, String msg) {
		ByteBuffer data = ByteBuffer.wrap(msg.getBytes());
        context.getConnector().sendDataPacket(tableId, context.getPlayerId(), data);
    }
