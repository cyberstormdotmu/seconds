...
public class StateClientReaper implements ClientReaper, StateClientReaperMBean {
    ...
    /** Abandoned node timeout. */
    private long abandonedNodeTimeout = 2 * 60000;
		...
		for (Node child : children) {
			...
			try {
				...
				if (isValidNodeWithData(child, ClientStatusFields.STATUS.name())) {
					...
					} else if (child.getData().containsKey(ClientStatusFields.ABANDONED.name())) {
					    checkAbandonedAndRemoveIfTimedOut(child);
					}
				} else if (isValidNodeWithoutData(child)) {
				    // node looks abandoned, mark as eligible for cleanup later
				    log.debug("Client node " + child.getFqn() + " looks abandoned, mark as eligible for cleanup after timeout");
				    cacheHandler.updateAttribute(child.getFqn().toString(), ClientStatusFields.ABANDONED.name(), "" + System.currentTimeMillis());
				}
			} catch (Exception e) {
    ...
    /**
     * Checks if the node is valid but empty (no data).
     * 
     * @param child
     * @return true if valid but empty
     */
    @SuppressWarnings("rawtypes")
    private boolean isValidNodeWithoutData(Node child) {
        return child != null  &&  child.getData() != null  &&  child.getData().isEmpty();
    }
	...
	private void checkRejoin(Node child, boolean force) {
		...
		if (waitTime > clientReconnectTimeout || force) {
			log.info("Client " + pid + " has timed out. Removing client.");
	...
	@SuppressWarnings("rawtypes")
    private void checkAbandonedAndRemoveIfTimedOut(Node child) {
        int pid = Integer.parseInt(child.getFqn().getLastElementAsString());
        long waitTime = System.currentTimeMillis() - (Long)child.getData().get(ClientStatusFields.ABANDONED.toString());
        
        if (child.getData().containsKey(ClientStatusFields.STATUS.name())) {
            child.getData().remove(ClientStatusFields.STATUS.name());
        } else if (waitTime > abandonedNodeTimeout) {
            log.debug("Client node " + child.getFqn() + " has been abandoned and will be removed.");
            registry.clientReaperTimeout(pid);
            reapedClients.incrementAndGet();
        }
	}
