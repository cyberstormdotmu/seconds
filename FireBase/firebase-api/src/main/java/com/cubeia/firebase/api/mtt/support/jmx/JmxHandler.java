...
public class JmxHandler {
	...
	/**
	 * Returns a stats mbean if it exists. Will NOT create a new one. 
	 * Use {@link #createStatsBean(MTTStateSupport)} to create new stats beans.
	 * @param state
	 * @param id
	 * @return
	 */
	public MttStats getStatsBean(MTTStateSupport state, int id) {
		return statBeans.get(id);
	}			
	
	/**
	 * Creates a new stats mbean.
	 * @param state
	 * @return
	 */
	public MttStats createStatsBean(MTTStateSupport state) {
		MttStats stats = null;
