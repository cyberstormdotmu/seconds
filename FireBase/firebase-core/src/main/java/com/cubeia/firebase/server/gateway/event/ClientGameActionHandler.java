...
public class ClientGameActionHandler extends DefaultActionVisitor {
	...
	public void handleAction(GameAction action) {
		...
		if (checkValidAction(action)) {
		    ...
		    log.debug("won't forward action to client for table not joined: playerId = " + client.getId() + ", action = " + action);
	...
	public void visit(JoinResponseAction action) {
        log.trace("join table process ended: tableId = " + action.getTableId() + ", status = " + action.getStatus());
        client.getLocalData().removeJoinTableRequestInProgress(action.getTableId());
...
//	}
	...
	private void handleUnknownTable(int tableId) {
		if (!client.getLocalData().isTableNotified(tableId)) {
			...
			// do not set player as WAITING_REJOIN if a join request is in progress
			if (!client.getLocalData().isJoinTableRequestInProgress(tableId)) {
			    ...
			    log.debug("join table in progress, won't set player to " + PlayerStatus.WAITING_REJOIN + ", tableId = " + tableId);
			}
