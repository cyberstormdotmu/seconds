...
public class GameOrphanMBusListener extends DefaultActionVisitor implements OrphanEventListener<ChannelEvent> {
	...
	public void visit(WaitingRequestAction action) {
		log.warn("Received waiting request for unreachable table: "+action);
		WaitingResponseAction response = new WaitingResponseAction(action.getPlayerId(), action.getTableId());
		createEvent(action.getPlayerId(), action.getTableId(), response);
	}
