...
public class ClientLocal {
	...
	/**
	 * Contains ids of tables that has recieved a join request but not yet responed.
	 */
    private Set<Integer> joinRequestInProgress = new ConcurrentHashSet<Integer>();
	...
	/**
	 * Marks the given table is as being in progress of joining.
	 * @param tableid table id
	 */
    public void addJoinTableRequestInProgress(int tableid) {
        joinRequestInProgress.add(tableid);
    }

    /**
     * Remove the table as being in progress of joining. This should be done then
     * the join is either accepted or denied or when the client is disconnected.
     * @param tableid
     */
    public void removeJoinTableRequestInProgress(int tableid) {
        joinRequestInProgress.remove(tableid);
    }

    /**
     * Returns true if the the process of joining the given table has begun but not finished.
     * @param tableid table id
     * @return true if joining table is in progress
     */
    public boolean isJoinTableRequestInProgress(int tableid) {
        return joinRequestInProgress.contains(tableid);
    }
}
