...
class HttpUtil {
    ...
    public static InetSocketAddress createSocketAddress(ServletUpgradeRequest req) {
	...
	public static boolean checkHandshake(ServletUpgradeRequest supReq, String handShake) {
	    if (handShake != null) {
            String theirs = getRequestHandshake(supReq);
	        return handShake.equals(theirs);
	    }
	    
	    return true;
	}
	
    public static String getRequestHandshake(ServletUpgradeRequest supReq) {
        String msg = supReq.getHeader(HANDSHAKE_HTTP_HEADER);
        if(msg == null) {
            msg = findHandshakeCookie(supReq);
        }
        if(msg == null) {
            List<String> values = supReq.getParameterMap().get(HANDSHAKE_HTTP_HEADER);
    ...
    private static String findHandshakeCookie(ServletUpgradeRequest supReq) {
        for (java.net.HttpCookie c : supReq.getCookies()) {
