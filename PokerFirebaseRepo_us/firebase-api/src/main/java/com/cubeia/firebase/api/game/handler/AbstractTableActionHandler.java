...
public abstract class AbstractTableActionHandler extends AbstractActionHandler {   
    ...
    public void visit(WatchAction action) {
        ...
        //boolean seatStatus = seating.getNumberOfSeats() > playerSet.getPlayerCount();
        int tableIsFull = 1;
        
        if(tableHasAnyFreeSeats(seating)){
        	tableIsFull = 0;
        }
        ...
        WatchResponseAction response = new WatchResponseAction(action.getTableId(), status, tableIsFull);
    ...
    public void visit(UnWatchAction action) {
        ...
        if (!action.isByServer()) {
        	...
        	// Tatvasoft - 05/27/2014 - Removed the player from waiting list if it Unwatch
        	ArrayList<TableWaiting> waitingList = StandardTableActionHandler.getGlobalWaitingList().get(action.getTableId());
        	
        	// If Waiting list contains entries for current table
        	
        	// Tatvasoft Code Starts ==> July 2nd, 2014
        	// This Logic For Removing Waiting Player from List when the waiting player leaves table
			if (waitingList != null && waitingList.size() > 0) {
				for (int i = 0; i < waitingList.size(); i++) {
					TableWaiting tableWaiting = waitingList.get(i);

					// If waiting player matches with current request player
					if (tableWaiting.getPlayer().getPlayerId() == action.getPlayerId()) {
						// Remove from waiting list
						waitingList.remove(i);
						StandardTableActionHandler.getGlobalWaitingList().put(action.getTableId(), waitingList);
						...
						if (waitingList == null || waitingList.size() == 0) {
							StandardTableActionHandler.getGlobalWaitingList().remove(action.getTableId());
  ...
  /*  protected WaitingResponseAction waitJoinResponse(int pid, int tid) {
        //Status status = accepted.equals("J") ? Status.OK : accepted.equals("R") ? Status.WAITING : Status.DENIED;
        //JoinResponseAction response = new JoinResponseAction(pid, tid, seat, status.ordinal());
        //String[] players = {"Sushant","Nirav","Chintan"};
        List<WaitingList> waitingLists = new ArrayList<WaitingList>();
        byte b1 = 30;
        byte b2 = 31;
        WaitingList waitingList1 = new WaitingList(1,"Sushant",b1);
        WaitingList waitingList2 = new WaitingList(2,"Nirav",b2);
        waitingLists.add(waitingList1);
        waitingLists.add(waitingList2);
        
        WaitingResponseAction responseAction = new WaitingResponseAction(pid, tid, waitingLists);
        //response.setResponseCode(responseCode);
        
        getNotifier().sendToClient(pid, responseAction);
        return responseAction;
    }
    
    */
    
    protected WaitingResponseAction notifyWaitingResponse(int pid,int tid){
    	List<TableWaiting> tableWaitingList = StandardTableActionHandler.getGlobalWaitingList().get(tid);
    	List<WaitingList> waitingLists = new ArrayList<WaitingList>();
    	
   	 	byte b1 = 1;
   	 	if(tableWaitingList != null){
	   	 	for (int i = 0; i < tableWaitingList.size(); i++) {
	   	 		WaitingList waiting = new WaitingList(tid,tableWaitingList.get(i).getPlayer().getName(),b1);
	   	 		waitingLists.add(waiting);
	   	 		//waitingplayers[i] = waitingList.get(i).getPlayer().getName();
	   	 	}
   	 	}
	    WaitingResponseAction response = new WaitingResponseAction(pid, tid,waitingLists);
	    return response;
    }
    
    
    protected WaitingResponseAction sendWaitingResponse(int pid,int tid){
    	List<TableWaiting> tableWaitingList = StandardTableActionHandler.getGlobalWaitingList().get(tid);
    	List<WaitingList> waitingLists = new ArrayList<WaitingList>();
     ...
     * Tatvasoft
     * 
     * Sends the response for join action
     * 
     * @param pid player id
     * @param tid table id
     * @param seat seat
     * @param accepted join request acceptance status
     * @param responseCode response code
     * @return join response
     */
    protected JoinResponseAction sendJoinResponse(int pid, int tid, int seat, String accepted, int responseCode) 
    {
    	/*
    	 * If accepted = J, status = OK
    	 * If accepted = R, status = WAITING
    	 * If accepted = D, status = DENIED
    	 */
        Status status = accepted.equals("J") ? Status.OK : accepted.equals("R") ? Status.WAITING : Status.DENIED;
        JoinResponseAction response = new JoinResponseAction(pid, tid, seat, status.ordinal());
        
        //waitJoinResponse(pid,tid);
        response.setResponseCode(responseCode);
