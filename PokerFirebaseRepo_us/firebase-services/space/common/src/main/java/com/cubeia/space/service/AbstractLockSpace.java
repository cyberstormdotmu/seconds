	...
	public E take(int id, Object action) {
		try {
			if (locks.lock(id, lockTimout, action)) {
				...
				log.error("Timeout or failure when waiting for write lock. Please check if some other thread in the system is hanging while accessing this resource. Space name: " + space.getName() + "; Object: " + id);
				return null;
			}
