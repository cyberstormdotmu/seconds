...
public class StateClientRegistry implements ClientRegistry, StateClientRegistryMBean {
	...
	public synchronized void logoutClient(Client client, boolean removeFromTables) {
		...
		if (tmp == null) {
	        ...
	        if (client.isClientConnectedRemotely()) {
	            log.debug("client shouldn't be killed, new one is already connected. Client id = " + client.getId());
	        } else {
        		if (removeFromTables) {
	...
	public boolean clientLostConnection(int id) {
		...
		if (client != null ) {
		    ...
		    log.trace("client " + id + " lost connection, will change status on all tables to " + PlayerStatus.WAITING_REJOIN);
	...
	public Integer getOperatorId(int playerId) {
		String fqn = SystemStateConstants.CLIENT_ROOT_FQN+playerId;
	...
	private void reportStatusChangeToAllTables(int playerId, PlayerStatus status) {
		...
		for (Object table: tables.keySet()) {
			...
			log.trace("reporting status change to table: tid = " + tableId + ", pid = " + playerId + ", status = " + status);
	...
	public void kickPlayer(int playerId) {
		...
		packet.code = ForcedLogoutCode.KICKED.ordinal(); 
	...
	public void clientLoggedInRemotely(int clientId) {
		...
		ForcedLogoutPacket packet = new ForcedLogoutPacket();
		// TODO: change to non hardcoded values  
		packet.code = ForcedLogoutCode.LOGGED_IN_REMOTELY.ordinal(); 
		...
		if (client != null) {
			client.sendClientPacket(packet);
			client.setClientConnectedRemotely(true);
	...
	private class KickClient implements Runnable {
		...
		public KickClient(StateClientRegistry registry, int playerId) { 
