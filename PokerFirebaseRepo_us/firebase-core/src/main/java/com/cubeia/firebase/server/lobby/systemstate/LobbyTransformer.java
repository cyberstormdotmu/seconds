...
public class LobbyTransformer {
    ...
    public static ProtocolObject transform(TableInfo table) {
        ...
        // Tatvasoft Modification Starts
     	// Update lobby for Number Of waiting Players based on request - response
        ArrayList<TableWaiting> waitingList = StandardTableActionHandler.getGlobalWaitingList().get(table.getId());
        if(waitingList!=null && waitingList.size()>0){
        	snapshot.noofwaitingplayers = waitingList.size();
        }else{
        	snapshot.noofwaitingplayers = 0;
        }
        // Tatvasoft Modification Ends
