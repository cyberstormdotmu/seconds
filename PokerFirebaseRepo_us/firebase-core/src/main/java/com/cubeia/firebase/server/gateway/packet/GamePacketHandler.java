...
public class GamePacketHandler extends AbstractPacketHandler {
    ...
    public void visit(JoinRequestPacket packet) {
        log.trace("join table in progress, tableId = " + packet.tableid);
        client.getLocalData().addJoinTableRequestInProgress(packet.tableid);
	...
	public void visit(WaitingList packet) {
		...
		System.out.println("Inside Waitinglist visit 2");
	}
	...
	public void visit(WaitingListPacket packet) {
		// TODO Auto-generated method stub
		System.out.println("Inside WaitinglistPacket visit 2");
	Check Here --->	WaitingResponseAction action = new WaitingResponseAction(getPlayerid(), packet.tableid);
		sendEvent(packet.tableid, action, packet);
	}*/
    ...
    public void visit(WaitingListPacket packet) {
		System.out.println("Inside WaitinglistPacket visit 2");
		WaitingRequestAction action = new WaitingRequestAction(getPlayerid(), packet.tableid);
		sendEvent(packet.tableid, action, packet);
    }
	
	@Override
    public void visit(WaitCheckPacket packet) {
		System.out.println("Inside WaitcheckPacket visit 2");
		WaitCheckPacketAction action = new WaitCheckPacketAction(getPlayerid(),packet.tableid);
		sendEvent(packet.tableid, action, packet);
    }
