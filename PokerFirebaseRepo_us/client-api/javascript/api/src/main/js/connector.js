...
FIREBASE.Connector = function(packetCallback, lobbyCallback, loginCallback, statusCallback) {
    ...
    var _handleLoginResponse = function(loginResponse) {
        if (_loginCallback) {
            ...
            _loginCallback(loginResponse.status, loginResponse.pid, loginResponse.screenname, loginResponse.credentials || null, loginResponse.authtoken, loginResponse.code);
        }
    };
...
//DEFINED_FOR_JASMINE_TESTING_END
    ...
    /**
     * Get Waiting list of given table.
     * @param {Number} tableId table id
     */
    this.waitList = function(tableId) {
        var waitListRequest = new FB_PROTOCOL.WaitingListPacket();
        waitListRequest.tableid = tableId;
        this.sendProtocolObject(waitListRequest);
    };
    //Tatvasoft Code Starts
    // Ssending Request Packet wih Table Id
    this.checkwaitingPlayer=function(tableId){var waitcheckPacket=new FB_PROTOCOL.WaitCheckPacket();waitcheckPacket.tableid=tableId;this.sendProtocolObject(waitcheckPacket);};
    //Tatvasoft Code Ends
