 ...
 */
public class AppointmentLateRpt extends AbstractITextPdfView {
	...
	public Document buildPdfDocument(Map<String, Object> model,
		...
		//get the list of late appointments
		List<AppointmentMaster> lateAppointmentMasterList = (List<AppointmentMaster>)model.get("lateAppointmentMasterList");
		String message = null;
		message = (String)model.get("message");
		
		// check whether the list contains data or not
		if(message == null){
		document.add(new Paragraph("List of Late Appointments"));
		document.add(new Paragraph(lateAppointmentMasterList.get(0).getAppointmentDate().toString()));
		
		// table layout for pdf
		PdfPTable table = new PdfPTable(8);
		table.setWidthPercentage(100.0f);
		table.setWidths(new float[] {0.5f, 1.0f, 1.5f, 1.5f, 1.0f, 1.0f, 1.5f, 1.0f});
		...
		for (AppointmentMaster appointmentMaster : lateAppointmentMasterList) {
