...
public class OperatorServiceImpl implements OperatorService {
	...
	public Map<OperatorConfigParamDTO, String> getOperatorConfigListForClients(Long operatorId) {
		...
		clientConfig.put(CURRENCIES,allConfig.get(OperatorConfigParamDTO.CURRENCIES));
        clientConfig.put(LOBBY_RIGHT_PROMOTION_URL,allConfig.get(OperatorConfigParamDTO.LOBBY_RIGHT_PROMOTION_URL));
        clientConfig.put(LOBBY_TOP_PROMOTION_URL, allConfig.get(OperatorConfigParamDTO.LOBBY_TOP_PROMOTION_URL));
        clientConfig.put(CLIENT_TITLE,allConfig.get(OperatorConfigParamDTO.CLIENT_TITLE));
        clientConfig.put(OPERATOR_NAME,allConfig.get(OperatorConfigParamDTO.OPERATOR_NAME));
	...
	public List<OperatorCurrencyDTO> getOperatorCurrencies(Long operatorId) {
	    String curJson = getOperatorConfig(operatorId, OperatorConfigParamDTO.CURRENCIES);
	    
	    List<OperatorCurrencyDTO> curList = new ArrayList<>();
	    try {
            curList = new ObjectMapper().readValue(curJson, new TypeReference<List<OperatorCurrencyDTO>>(){});
        } catch (Exception e) {
            log.error("unable to parse json as list of currencies: '{}'", curJson);
        } 
        return curList;
	}
}