...
public class UserDAOImpl implements UserDAO {
	...
	public String getUserPhotoUrl(Long userId) 
	{
		...
		String photoUrl = user.getInformation().getPhoto();
		String hql = "select u.photo from UserInformation u where u.id = :id";  
		String photo = null;
		List<String> userNameList = em.createQuery(hql).setParameter("id", userId).getResultList();
		if(userNameList != null && userNameList.size() > 0 && userNameList.get(0) != null) {
			photo = userNameList.get(0).toString();
		}
    	return photo;
		//return photoUrl;
	}
