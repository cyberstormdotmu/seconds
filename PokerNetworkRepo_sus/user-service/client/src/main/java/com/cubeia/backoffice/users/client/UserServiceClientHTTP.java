...
public class UserServiceClientHTTP implements UserServiceClient {
	...
	private static final String PHOTO_URL = USER+"/photourl";
	...
	public UserServiceClientHTTP() {
		this("https://userservice:9090/user-service-rest/rest");
	}
    ...
    public String ping() {
        ...
        try {
            ...
            throw new RuntimeException("Failed pinging: " + baseUrl + PING, e);
        } finally {
	...
	public AuthenticationResponse authenticate(Long operatorId, String username, String password) {
		...
		try {
			...
			throw new RuntimeException("Failed authenticating via: " + baseUrl + AUTHENTICATE, e);
		} finally {
			method.releaseConnection();
		}
	}
	...
	public AuthenticationResponse authenticateSessionToken(String sessionToken) {
		...
		try {
			...
			throw new RuntimeException("Failed authenticating session token via: " + baseUrl + AUTHENTICATE_TOKEN, e);
		} finally {
			method.releaseConnection();
		}
	}
	...
	public void invalidateSessionToken(Long userId) {
		...
		try {
			...
			throw new RuntimeException("Failed invalidating session token via: " + resource, e);
		} finally {
			method.releaseConnection();
		}
	}
	...
	public User getUserById(Long userId) {
		...
		try {
			...
			throw new RuntimeException("Failed getting user by id via resource: " + resource, e);
		} finally {
			method.releaseConnection();
		}
	}
    ...
    public User getUserByExternalId(String externalId, Long operatorId) {
        ...
        try {
            ...
            throw new RuntimeException("Failed getting user by external id via resource: " + resource, e);
        } finally {
            method.releaseConnection();
        }
    }
	...
	public Map<String, String> getUserAttributes(Long userId) {
		...
		try {
			...
			throw new RuntimeException("Failed getting user attributes via resource: " + resource, e);
		} finally {
			method.releaseConnection();
		}
	}
	...
	public String getUsername(Long userId) {
		...
		try {
			...
			throw new RuntimeException("Failed getting username via resource: " + resource, e);
		} finally {
			method.releaseConnection();
		}
	}
	...
	{
		String resource = String.format(baseUrl+PHOTO_URL, userId);
		GetMethod method = createGetMethod(resource);
