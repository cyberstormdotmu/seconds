...
public class WalletAccountingManagerImpl implements WalletAccountingManager {
	...
	@Override
    public Collection<Account> getAccountsByTypeAndOperatorIdAndCurrency(String accountType, Long operatorId, String currencyCode) {
	      return manager.getAccountsByTypeAndOperatorIdAndCurrency(accountType, operatorId, currencyCode);
    }
	
	@Override
    public Account getAccountByTypeAndOperatorIdAndCurrency(String accountType, Long operatorId, String currencyCode) {
          Collection<Account> accounts = manager.getAccountsByTypeAndOperatorIdAndCurrency(accountType, operatorId, currencyCode);
          if (accounts.isEmpty()) {
              return null;
          } else if (accounts.size() > 1) {
              throw new RuntimeException("multiple accounts found for operator id = " + operatorId + " and currency = "+currencyCode);
          } else {
              return accounts.iterator().next();
          }
    }
	...
	public Account getAccountByUserId(Long userId) {
		...
		if (accounts.isEmpty()) {
		    ...
		    throw new RuntimeException("multiple accounts found for user id = " + userId);
		} else {
		    return accounts.iterator().next();
		}
	}
	...
	public Map<Long, Account> getAccounts(Set<Long> ids) {
		return manager.getAccounts(ids);
	}

	@Override
	public void setAllowTransactionsWithClosedAccounts(boolean allow) {
	    manager.setAllowTransactionsWithClosedAccounts(allow);
	}

	@Override
	public Account createAccountWithInitialBalance(Account account,
			BigDecimal balance) {
		return manager.createAccountWithInitialBalance(account, balance);
	}

	@Override
	public Collection<Account> getAccountsByUserId(Long userId) {
		return manager.getAccountsByUserId(userId);
	}
	...
	public Collection<Account> getAccountsByUserAndWalletId(Long userId,
		...
		return manager.getAccountsByUserAndWalletId(userId, walletId);
	}

	@Override
	public Collection<Account> getAccountsByUserIdWalletTypeAndCurrency(
			Long userId, Long walletId, String type, String currency) {
		return manager.getAccountsByUserIdWalletTypeAndCurrency(userId, walletId, type, currency);
	}

	@Override
	public Collection<Account> getAccountsByUserIdAndCurrency(Long userId,
			String currency) {
		return manager.getAccountsByUserIdAndCurrency(userId, currency);
	}
	...
	public Transaction reverseTransaction(Long txId, String newExternalId,
			...
			throws TransactionNotFoundException {
		return manager.reverseTransaction(txId, newExternalId, oldAttr, newAttr);
	}
	...
	public QueryResultsContainer<Account> listAccounts(Long accountId,
		...
		return manager.listAccounts(accountId, userId, walletId, currencyCode, 
		    entityStatuses, entityTypes, offset, limit, order, ascending);
	}
	...
	public QueryResultsContainer<Transaction> listTransactionsByAttribute(Long accountId, Long userId, String key,
	    ...
	    return manager.listTransactionsByAttribute(accountId, userId, key, value, offset, limit, order, ascending);
	}
	...
	public List<CurrencyRate> listRatesForCurrency(String currencyCode,
		...
		return manager.listRatesForCurrency(currencyCode, date, maxAgeDays);
	}

	@Override
	public void setCheckpointExecTimeCollector(ExecTimeCollector col) {
	    manager.setCheckpointExecTimeCollector(col);
	}

	@Override
	public boolean getAllowTransactionsWithClosedAccounts() {
		return manager.getAllowTransactionsWithClosedAccounts();
	}

	@Override
	public boolean getAsyncCheckpointCreation() {
		return manager.getAsyncCheckpointCreation();
	}

	@Override
	public int getBalanceCheckpointInterval() {
		return manager.getBalanceCheckpointInterval();
	}

	@Override
	public Transaction getTransactionByExternalId(String extId) {
		return manager.getTransactionByExternalId(extId);
	}

    @Override
    public Account findUniqueAccount(AccountQueryRequest request) {
        return manager.findUniqueAccount(request);
    }
