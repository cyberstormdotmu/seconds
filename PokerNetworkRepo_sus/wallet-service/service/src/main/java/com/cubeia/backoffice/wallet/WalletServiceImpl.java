...
public class WalletServiceImpl implements WalletService {
	...
	public void updateAccount(com.cubeia.backoffice.wallet.api.dto.Account account) {
		...
		accountById.setOperatorId(account.getOperatorId());
	...
	public CreateAccountResult createAccount(CreateAccountRequest request) {
		...
		if (currency == null) {
			throw new RuntimeException("Create account failed. Currency (" + request
					.getCurrencyCode() + ") is not supported. Supported currencies are: " + getSupportedCurrencies());
		}

		Account account = new Account(currency.getCode(), currency.getFractionalDigits());
		account.setUserId(request.getUserId());
		account.setOperatorId(request.getOperatorId());
	...
	public AccountBalanceResult getAccountBalance(long accountId) throws AccountNotFoundException {
		...
		if (account != null) {
			...
			balanceResponse.setBalance(new Money(account.getCurrencyCode(), account.getFractionalDigits(), balance.getAmount()));
	...
	@Override
    public com.cubeia.backoffice.wallet.api.dto.Account findUniqueAccount(AccountQuery query) {
        AccountQueryRequest request = new AccountQueryRequest();
        request.setAttributes(query.getAttributes());
        request.setCurrency(query.getCurrency());
        request.setOperatorId(query.getOperatorId());
        request.setUserId(query.getUserId());
        request.setType(query.getType());
        
	    Account account = accountingManager.findUniqueAccount(request);
	    
	    com.cubeia.backoffice.wallet.api.dto.Account dto = dtoFactory.createAccountDTOFromEntity(account);
        return dto;
    }

	@Override
	public AccountQueryResult listAccounts(Long accountId, Long userId, Collection<com.cubeia.backoffice.wallet.api.dto.Account.AccountStatus> statuses,
			Collection<com.cubeia.backoffice.wallet.api.dto.Account.AccountType> types, int offset, int limit, AccountsOrder order, boolean ascending) {
		...
		QueryResultsContainer<Account> resultContainer = accountingManager
				.listAccounts(accountId, userId, entityStatuses, entityTypes, offset, limit, dtoFactory.createAccountsOrderEntityFromDTO(order), ascending);

		return new AccountQueryResult(offset, limit, resultContainer.getTotalQueryResultSize(),
				dtoFactory.createAccountDTOsFromEntities(resultContainer.getResults()), order, ascending);
	}

	@Override
	public TransactionQueryResult listTransactions(Long account1Id, Boolean account1credit, boolean id1IsUserId, Long account2Id, Boolean account2credit,
			boolean id2IsUserId, Date startDate, Date endDate, int offset, int limit, TransactionsOrder order, boolean ascending) {

		QueryResultsContainer<Transaction> resultContainer = accountingManager
				.listTransactions(TransactionParticipant.legacy(account1Id, account1credit, id1IsUserId),
						TransactionParticipant.legacy(account2Id, account2credit, id2IsUserId), startDate, endDate, offset, limit,
						dtoFactory.createTransactionOrderEntityFromDTO(order), ascending);

		ArrayList<com.cubeia.backoffice.wallet.api.dto.Transaction> txDTOs = new ArrayList<com.cubeia.backoffice.wallet.api.dto.Transaction>();
		for (Transaction tx : resultContainer.getResults()) {
			txDTOs.add(dtoFactory.createTransactionDTOFromEntity(tx));
		}

		return new TransactionQueryResult(offset, limit, resultContainer.getTotalQueryResultSize(), txDTOs, order, ascending);
	}
	
	@Override
	public TransactionQueryResult listTransactionsByAttribute(Long accountId, Long userId, String key, String value,
	    int offset, int limit, TransactionsOrder order, boolean ascending) {
	    
	    QueryResultsContainer<Transaction> resultContainer = accountingManager
	        .listTransactionsByAttribute(accountId, userId, key, value, 
	...
	public EntriesQueryResult listEntries(Long accountId, boolean includeBalances, int offset, int limit, boolean ascending) {
		...
		QueryResultsContainer<Entry> resultContainer = accountingManager.listEntries(accountId, offset, limit, ascending);

		ArrayList<com.cubeia.backoffice.wallet.api.dto.Entry> entryDTOs = new ArrayList<com.cubeia.backoffice.wallet.api.dto.Entry>();
	...
	public com.cubeia.backoffice.wallet.api.dto.Transaction getTransactionById(Long txId) {
		...
		return convertToDtoAndFillInBalances(tx);
	}

    @Override
    public com.cubeia.backoffice.wallet.api.dto.Transaction getTransactionByExternalId(String xId) {
        Transaction tx = accountingManager.getTransactionByExternalId(xId);
    ...
    private com.cubeia.backoffice.wallet.api.dto.Transaction convertToDtoAndFillInBalances(Transaction tx) {
	...
	public com.cubeia.backoffice.wallet.api.dto.Account getStaticAccountForCurrency(Long userId, String currencyCode) {
		return dtoFactory
				.createAccountDTOFromEntity(accountingManager.getAccountByUserIdTypeAndCurrency(userId, AccountType.STATIC_ACCOUNT.toString(), currencyCode));
	}
	...
	@Override
    public Long getOrCreateOperatorAccountIdForCurrency(Long operatorId, String currencyCode) {
        return getOrCreateOperatorAccountForCurrency(operatorId, currencyCode).getId();
    }
	
	private Account getOrCreateOperatorAccountForCurrency(Long operatorId, String currencyCode) {
	    Account operatorAccount = accountingManager.getAccountByTypeAndOperatorIdAndCurrency(AccountType.OPERATOR_ACCOUNT.name(), operatorId, currencyCode);
        if (operatorAccount == null) {
            ...
            operatorAccount = new Account(null, currencyCode, currency.getFractionalDigits());
            operatorAccount.setOperatorId(operatorId);
            operatorAccount.setType(AccountType.OPERATOR_ACCOUNT.name());
            accountingManager.createAccount(operatorAccount);
        }
	...
	public void handleTransferFunds(com.cubeia.backoffice.wallet.api.dto.Account account, TransferRequest request) {
		log.info("Handling transfer " + request + " account: " + account);
	...
	private void handleTransferWithLocalAccount(com.cubeia.backoffice.wallet.api.dto.Account account, TransferRequest request) {
		...
		if (request.getTransferType() == TransferType.CREDIT) {
			...
			accountingManager.createTransaction(request.getComment(), null, request.getAmount(), staticAccountId, account.getId(), null);
			...
			accountingManager.createTransaction(request.getComment(), null, request.getAmount(), account.getId(), staticAccountId, null);
	...
	private void handleTransferWithRemoteWallet(com.cubeia.backoffice.wallet.api.dto.Account account, TransferRequest request) {
		...
		Money amount = new Money(getCurrency(account.getCurrencyCode()), request.getAmount());
		if (request.getTransferType() == TransferType.CREDIT) {
	...
	private java.util.Currency getCurrency(String currencyCode) {
		try {
			return java.util.Currency.getInstance(currencyCode);
		} catch (IllegalArgumentException e) {
			throw new IllegalArgumentException("Failed creating java.util.Currency for currency code: " + currencyCode);
		}
	}
	...
	public Collection<com.cubeia.backoffice.wallet.api.dto.Currency> getSupportedCurrencies() {
		List<com.cubeia.backoffice.wallet.api.dto.Currency> currencies = new ArrayList<com.cubeia.backoffice.wallet.api.dto.Currency>();
