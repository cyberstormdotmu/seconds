...
public class WalletServiceClientHTTP implements WalletServiceClient {
	...
	private static final String UNIQUE_ACCOUNT_QUERY = WALLET+"/accounts/unique";
	private static final String TRANSACTIONS = WALLET+"/transactions";
    private static final String TRANSACTIONS_BY_ATTRIBUTE = WALLET+"/transactions/attribute";
	...
	private static final String LOOKUP_ACCOUNT_BALANCES = WALLET+"/balances/%s";

	private static final String OPERATOR_ACCOUNT_ID = WALLET+"/operator/%s/%s";
	
	private static final String TRANSACTION_BY_ID = WALLET+"/transaction/id/%s";
    private static final String TRANSACTION_BY_XID = WALLET+"/transaction/xid/%s";
	...
	@Override
	public List<AccountBalanceResult> getAccountBalancesByUser(Long userId) {
		String resource = String.format(baseUrl + LOOKUP_ACCOUNT_BALANCES, userId);
		...
		try {
			...
			InputStream body = method.getResponseBodyAsStream();
			try {
				CollectionType collectionType = mapper.getTypeFactory().constructCollectionType(List.class, AccountBalanceResult.class);
			...
			if (method.getStatusCode() == 404) {
				...
				throw new RuntimeException("Failed getting account balance via resource: " + resource + ". Status code: " + method.getStatusCode(), e);
			}
		} catch (Exception e) {
			throw new RuntimeException("Failed getting account balance via resource: " + resource, e);
		} finally {
			method.releaseConnection();
		}
	}
	...
	public void transfer(Long accountId, TransferRequest request) {
		...
		try {
            ...
            log.debug("TransferRequest failed: {}, {}", resource, request);
			throw new RuntimeException("TransferRequest failed for: " + resource ,e);
		} finally {
			method.releaseConnection();
		}
	}
	...
	public Transaction getTransactionById(Long transactionId) {
		String resource = String.format(baseUrl+TRANSACTION_BY_ID, transactionId);
		GetMethod method = createGetMethod(resource);
    ...
    @Override
    public Transaction getTransactionByExternalId(String externalTxId) {
        String resource = String.format(baseUrl+TRANSACTION_BY_XID, externalTxId);
        GetMethod method = createGetMethod(resource);
    ...
    @Override
    public Account findUniqueAccount(AccountQuery request) {
        String uri = baseUrl + UNIQUE_ACCOUNT_QUERY;
        ...
        try {
            ...
            if (statusCode == HttpStatus.SC_BAD_REQUEST) {
                throw new RuntimeException("Found more than one account for query");
            }
            if (statusCode == HttpStatus.SC_OK) {
                InputStream body = method.getResponseBodyAsStream();
                return parseJson(body, Account.class);
                ...
                throw new RuntimeException("Failed to get unique account, RESPONSE CODE: " + statusCode + " url: " + uri);
            }
        } catch (Exception e) {
            throw new RuntimeException("Failed to get unique account via url " + uri, e);
        } finally {
            method.releaseConnection();
        }
    }
    ...
    @Override
    public TransactionQueryResult listTransactionsByAttribute(ListTransactionsByAttributeRequest request) {
        String uri = baseUrl + TRANSACTIONS_BY_ATTRIBUTE;
        PutMethod method = new PutMethod(uri);
        try {
            String data = serialize(request);
            method.setRequestEntity(new StringRequestEntity(data, MIME_TYPE_JSON, DEFAULT_CHAR_ENCODING));
	...
	private void parseJson(InputStream data, CollectionType collectionType) {
		try {
			Object o = mapper.readValue(data, collectionType);
		} catch (Exception e) {
			throw new RuntimeException("Error parsing json for collection " + collectionType, e);
		}
	}
	...
	private String serialize(Object value) {
		try {
			...
			throw new RuntimeException("Error serializing json for value " + value, e);
		}
	}

    @Override
    public Account getOperatorAccount(Long operatorId, String currencyCode) {
        String resource = String.format(baseUrl+OPERATOR_ACCOUNT_ID, operatorId, currencyCode);
        GetMethod method = createGetMethod(resource);
