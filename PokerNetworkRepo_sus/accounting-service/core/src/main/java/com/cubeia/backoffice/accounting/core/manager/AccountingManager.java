...
public interface AccountingManager {
    ...
    /**
     * List transactions by attribute. A null of empty filter parameter 
     * is a wild card. 
     * 
     * IMPORTANT: The key can NOT be left as a wild card as matching against attribute
     * value only is not supported.
     * 
     * <p>The result set is paged, for all results use 0/Integer.MAX.
     * 
     ...
     * @return The matching transactions and the size of the total result set size
     */
    QueryResultsContainer<Transaction> listTransactionsByAttribute(
        Long accountId, Long userId, String key, String value, 
	...
	Collection<Account> getAccountsByTypeAndOperatorIdAndCurrency(String accountType, Long operatorId, String currencyCode);
	
	/**
	 * Find a unique account by query.
	 * 
	 * @param request
	 * @throws Bad Request if more than one account was find using the query
	 * @return Account or Null if nothing found
	 */
	 Account findUniqueAccount(AccountQueryRequest request);
