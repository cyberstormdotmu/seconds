...
public class UserSummary extends BasePage {
    ...
    @SpringBean
    private UserPluginService pluginService;
    ...
    public UserSummary(PageParameters params) {
        ...
        List<UserPlugin> plugins = pluginService.getUserPlugins();
        if(plugins!=null && plugins.size()>0) {
            RepeatingView view = new RepeatingView("pluginPanels");
            for(UserPlugin p : plugins) {
                AbstractItem item = new AbstractItem(view.newChildId());
                item.add(p.createUserPanel("userPluginPanel",userId));
                view.add(item);
            }
            add(view);
        } else {
            add(new WebMarkupContainer("pluginPanels").add(new WebMarkupContainer("userPluginPanel")));
        }
    }
